...
;( function( window ) {
	...
	FForm.prototype._initEvents = function() {
		...
		this.ctrlContinue.addEventListener( 'click', function() {
			// Fix Pais
			if($('#pli').hasClass('fs-current') && $('#pais_id').val() != '1'){
				self._showField(3);
			}else{
				self._nextField();
			}
	...
	FForm.prototype._showError = function( err ) {
		...
		switch( err ) {
			case 'NOVAL' :
				message = 'Por favor completa el campo requerido para continuar';
				break;
			case 'INVALIDEMAIL' :
				message = 'Por favor completa tu dirección de mail para continuar';
				break;
			// ...
		};
...
function jugadores () {
    $('.more').click(function  () {
        ...
        if($('#player'+name).hasClass('escondido')){
            $('#rbox-'+name).addClass('fix');
            $('#player'+name).removeClass('escondido');
            $(this).addClass('long');          
            $('#player'+name).addClass('visible');
            $('#player'+name).effect( "drop",{direction: "up"}, 2000 );
        }else{          
            $('#player'+name).removeClass('visible');
            $('#player'+name).addClass('escondido');
            $('#rbox-'+name).removeClass('fix');
              $(this).removeClass('long');
            $('#player'+name).effect( "drop",{direction: "down"}, 2000 );
        }
...
function selectorPlay () {
    ...
    // start selects
     $(".fpais").selecter({
        customClass: "custom fpais",
        callback: filterSelect
    });

      $(".fprov").selecter({
        customClass: "custom fprov",
        callback: filterSelect
    });

    $(".fedad").selecter({
        customClass: "custom fedad",
        callback: filterSelect
    });

    $(".falt").selecter({
        customClass: "custom falt",
        callback: filterSelect
    });
    ...
    $('#fpas').click(function  () {      
        filterSelect('pasaporte', '1');            
    });

    $('#fvid').click(function  () {      
        filterSelect('video', '1');            
    });
    // start cancels
    // $('#croxpais').click(function  () {
    //    fpais = "off";
    //    esconder('croxpais');
    //    changeSel('fpais');       
    //    removeMark('markPais');
    //    swap(fpais, fprov, fedad, falt, fpeso, fpas, fvid);
    // });

   $('#cfilters').click(function  () {
      // pais
      fpais = 'off';
      removeMark('markPais');
      changeSel('fpais'); 
      ...
      fpas = 'off';
      removeMark('markPas');      
      $('#fpas').removeClass('marked');
      $('#sucP').removeClass('visible');
      // video
      fvid = 'off';
      removeMark('markVid');
      $('#fvid').removeClass('marked');
      $('#sucV').removeClass('visible');

        

      fvid = 'off'; swap(fpais, fprov, fedad, falt, fpeso, fpas, fvid);
   })
...
function changeSel(id) {
  $('#'+id).selecter("enable", "0");
  $('#'+id).val('0').trigger("change");
}
function selArg() {

  $('#fpais').val('1').trigger("change");
}

function swap (pais, prov, edad, alt, peso, pas, vid) {
    ...
    $( ".flags" ).each(function( index ) {
     ...
     $.ajax({
           ...
           url: '/traerJugadores',
           async: true,
           data: {fpais: pais, fprov: prov, fedad: edad, falt: alt, fpeso:peso, fpas:pas, fvid:vid},
           success: function(data) {            
                $("#jugList").html(data).show("slow");  
               
            }
       });

    
     if(pais != 'off' || prov != 'off' || edad != 'off' || peso != 'off' || alt != 'off' || pas != 'off' || vid != 'off' ){
        showClearFilters();       
     }else{
       hideClearFilters();
     }
...
function showClearFilters () {
   $('#clearFilters').removeClass('escondido');
   $('#clearFilters').addClass('visible');
}
function hideClearFilters () {
   $('#clearFilters').addClass('escondido');
   $('#clearFilters').removeClass('visible');
}

function addmark (id) {
  $('#marks').append("<div class='flags' id='"+id+"'> </div>")
}
...
function filterSelect(value, index) {
   ...
   // altura
   if($('.falt').hasClass('open')){
       falt = fval;
       swap(fpais, fprov, fedad, falt, fpeso, fpas, fvid);             
       addmark('markAlt');         
     }
   // peso
   if($('.fpeso').hasClass('open')){
         fpeso = fval;
        swap(fpais, fprov, fedad, falt, fpeso, fpas, fvid);    
        addmark('markPeso');       
    }
    ...
    if(value == 'pasaporte'){
        ...
        if($('#fpas').hasClass('marked')){          
          ...
          $('#fpas').addClass('marked');          
          $('#sucP').addClass('visible');          
          fpas = "1";        
          addmark('markPas');  
          swap(fpais, fprov, fedad, falt, fpeso, fpas, fvid);         
    ...
    if(value == 'video'){
        ...
        if($('#fvid').hasClass('marked')){          
          ...
          $('#fvid').addClass('marked');     
          $('#sucV').addClass('visible');      
          fvid = "1";        
          addmark('markVid');  
          swap(fpais, fprov, fedad, falt, fpeso, fpas, fvid);         
...
function crox (form) {
    ...
    var form2 = form.getAttribute('id');
